...
interface MapContainerProps {
  ...
  mapStyle: string;
  container: HTMLDivElement | null; // Allow null
...
export default function MapContainer({ mapboxAccessToken, mapStyle, container, viewerRef, positionMarkerRef, setImageId }: MapContainerProps) {
  ...
  useEffect(() => {
    console.log('MapContainer useEffect: container=', container, 'initialized=', initializedRef.current);
    if (!container) {
      console.warn('MapContainer: container is null, waiting for next render');
      return;
    }
    if (initializedRef.current) {
      console.log('MapContainer: already initialized, skipping');
      return;
    }
    ...
    containers.map.style.width = '100vw';
    ...
    async function initializeMap() {
      ...
      try {
        ...
        console.log('MapContainer: geocoder added');
        ...
        setTimeout(() => {
          ...
          if (geocoderEl) {
            ...
            console.warn('MapContainer: geocoder element not found');
          }
        }, 0);
        ...
        locateButton.addEventListener('click', () => {
          ...
          navigator.geolocation.getCurrentPosition(
            ...
            (error) => {
              console.error('MapContainer: Geolocation error:', error);
        ...
        geocoder.on('result', async (e: any) => {
          ...
          console.log('MapContainer: Geocoder result', e.result);
          ...
          try {
            ...
            console.error('MapContainer: Geocoder error:', error);
        ...
        geocoder.on('error', () => {
          console.error('MapContainer: Geocoder search failed');
          ...
          mapRef.current!.on('click', debounce(async (event: mapboxgl.MapMouseEvent) => {
            try {
              ...
              if (imageId && imageId.startsWith('fallback-')) {
                ...
                console.warn('MapContainer: Viewer not initialized on map click');
              }
              ...
              console.error('MapContainer: Error in map click handler:', error);
